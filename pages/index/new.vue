		...
		<view class="newview1" v-else v-for="(item,index) in listdata" :key="index">
			<view style="padding: 0 30rpx;">
				...
				<view class="plview">
					<u-row gutter="16">
						<u-col span="4">
							<view class="demo-layout bg-purple"style="text-align: center;" v-if="item.dz!=1" @click="dzlist(item,index)">
								<u-icon name="thumb-up" color="#000" size="30"></u-icon>
								<text style="margin-left: 10rpx;font-size: 13px;" >点赞({{item.dzmun}})</text>
								
							</view>
							<view class="demo-layout bg-purple" style="text-align: center;" v-if="item.dz==1" @click="cleandzlist(item,index)">
						...
						<u-col span="4">
							<view class="demo-layout bg-purple-dark" style="text-align: center;" v-if="item.sc!=1" @click="scclick(item,index)">
				...
				if(this.userinfo){
					...
					.then( res =>{
						...
						if(res.code ==200){
							...
							dzlist[index].dz = 0
							dzlist[index].dzmun=listdata[index].dzmun-1;
